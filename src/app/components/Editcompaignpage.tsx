...
const EdiCompaignPage: React.FC<Props> = ({ campaign_id, }: Props) => {
  ...
  const getData = async () => {
    ...
    const rect = await fetch(`http://localhost:3001/api/setrect`, {
      method: "POST",
      body: rectangleData
    })
  ...
  const handleClosePopup = () => {
    navigator.clipboard.writeText(`http://localhost:3001/${compaignDetails.campaign_name}`)
    setIsOpen(false);
  };
  return (
    <>
      <div className="flex flex-col">
        <ShareLink isOpen={isOpen} onClose={handleClosePopup} shareLink={`http://localhost:3001/${compaignDetails.campaign_name}`} />
